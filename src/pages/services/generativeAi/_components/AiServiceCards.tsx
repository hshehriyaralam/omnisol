...
const serviceData = [
  {
    ...
    description:
      "Get a clear strategy for how generative AI fits into your business. From use-case discovery to implementation roadmaps, we guide every step.",
  ...
  useEffect(() => {
    if (isInView) {
      controls.start({
        ...
        transition: {
          duration: 20, // Adjusted duration for smoother loop
          ease: "linear",
          repeat: Infinity,
          repeatType: "loop",
        },
    ...
    <div ref={ref} className="relative w-full overflow-hidden bg-white py-12 select-none">
      ...
      >
        ...
        {serviceData.map((service, idx) => (
          ...
          >
            <div className="p-4 flex flex-col max-w-[240px] mt-3">
              <h3 className="font-main font-medium text-[20px] leading-[28px] text-AiCardText mb-2">
                {service.title}
              </h3>
              <p className="font-main font-normal text-[13px] leading-[20px] text-AiCardText opacity-80">
                {service.description}
              </p>
            </div>
